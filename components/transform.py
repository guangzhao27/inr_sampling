...
class Transform(object):
    def __init__(self, args):
        ...
        self.method = args.transform.transform
    ...
    def _encode_shift_scale(self, data):
        data = data +  self.args.transform.trans_shift
        data = data * self.args.transform.trans_scale
        return data
    ...
    def _sym_power_trans(self, data):
        ...
        if self.args.transform.pn_beta <= 0:
            gamma = self._get_gamma_by_half(data)
        else:
            gamma = self._get_gamma_by_edge_claibration(data)
        
        boundary = self.args.transform.gamma_boundary
        if gamma > 1: gamma = min(boundary, gamma)
        else: gamma = max(1/boundary, gamma)


        if self.args.transform.pn_buffer < 0: # adaptive
            _alpha_len = int(self.args.transform.pn_alpha * 256)
            left_alpha_sum = pdf[:_alpha_len].sum()
            right_alpha_sum = pdf[-_alpha_len:].sum()
            _left_shift_len = self.args.transform.pn_k * left_alpha_sum
            _right_shift_len = self.args.transform.pn_k * right_alpha_sum
        
        else: 
            _left_shift_len = (_max - _min) * self.args.transform.pn_buffer
            _right_shift_len = (_max - _min) * self.args.transform.pn_buffer
    ...
    def _get_gamma_by_half(self, data):
        ...
        half_index = torch.searchsorted(cdf, self.args.transform.pn_cum)  # 0.5
    ...
    def _get_gamma_by_edge_claibration(self, data):
        ...
        _beta_len = int(self.args.transform.pn_beta * 256)
        ...
        delta_sum /= self.args.transform.pn_beta
        if half_gamma < 1: 
    ...
    def _box_cox(self, data: torch.tensor):
        ...
        data = data + self.args.transform.box_shift * 256
        _shape = data.shape
    ...
    def _inverse_box_cox(self, _min, _max, _lambda, normaed):
        ...
        r_data = normaed - self.args.transform.box_shift * 256
        return r_data
