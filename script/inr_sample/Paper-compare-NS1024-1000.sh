...
# Under sbatch, BASH_SOURCE may point to a Slurm spool copy; prefer submit dir.
REPO_ROOT="${SLURM_SUBMIT_DIR:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
if [[ ! -d "${REPO_ROOT}/inr_sample" ]]; then
  REPO_ROOT="/sdcc/u/gzhao/scratch/inr_sampling"
fi
...
if [[ $HOST == *"bnl"* ]]; then
  ...
  cd "$REPO_ROOT"
  wandb offline
  wandb_base_dir="${REPO_ROOT}/coral/wandb"
  export WANDB_DIR="$wandb_base_dir"
  wandb_run_dir="$wandb_base_dir/wandb"
  offline_log_file="${REPO_ROOT}/coral/wandb/offline_run_paths.txt"
  command_log_file="${REPO_ROOT}/offline_command_paths.sh"
  mkdir -p "$wandb_run_dir"
  touch "$offline_log_file"
  touch "$command_log_file"
  data_path="${REPO_ROOT}/data/NS2d/ns_data_res2048_re${re}_7.npy"
else
