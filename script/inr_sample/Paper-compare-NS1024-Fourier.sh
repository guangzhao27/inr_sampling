...
for seed in 42; do
for lr in 1e-5; do
for adaptive_grid_update_interval in 700; do
for adaptive_iterations in 3 4; do
  for case_name in \
    ...
    adaptive_grid_update_interval=$adaptive_grid_update_interval
    ...
    run_name="NS1024_sgd_lrtune_${case_name}_re_${re}_sampling_${sampling_rate}_lr_${lr}_depth_${depth}_t${time_frame}_ada_iter_${adaptive_iterations}_ada_update_${adaptive_grid_update_interval}"
