...
for time_frame in 100; do
  for sample_type in 2d_grid_adaptive; do
    ...
    run_name="NS1024_single_${sample_type}_re_${re}_sampling_${sampling_rate}_lr_${lr}_depth_${depth}_t${time_frame}_${adaptive_weight_mode}_notsamenumbercluster"
...
# salloc --account=csiml -p csi -q csi --gres=gpu:1  --cpus-per-task=1 -t 01:00:00 -n 1