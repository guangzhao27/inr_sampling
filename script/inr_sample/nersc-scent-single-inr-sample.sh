...
for time_frame in 100; do
  for sample_type in 2d_grid_adaptive random 2d_grid_linear; do
    for adaptive_weight_mode in none; do
      for model_type in single_image_fourier_mlp; do
        for optimizer_name in sgd; do
    run_name="NS1024_${model_type}_single_${sample_type}_opt_${optimizer_name}_re_${re}_sampling_${sampling_rate}_lr_${lr}_depth_${depth}_t${time_frame}_${adaptive_weight_mode}with512"
