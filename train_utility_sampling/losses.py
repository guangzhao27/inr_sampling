...
def relative_rmse(pred: torch.Tensor, target: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """
    Computes the relative Root Mean Squared Error (relative RMSE) between predicted and target tensors.

    Args:
        pred (torch.Tensor): The predicted tensor (e.g., reconstructed features).
        target (torch.Tensor): The ground truth tensor (e.g., original features).
        eps (float): A small value to avoid division by zero.

    Returns:
        torch.Tensor: The relative RMSE value.
    """
    mse = ((pred - target) ** 2).mean()
    norm = (target ** 2).mean()
    rel_rmse = torch.sqrt(mse / (norm + eps))
    return rel_rmse
