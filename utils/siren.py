...
class ModulatedSiren(Siren):
    ...
    def sample(self, graph, sample_params):
        sample_type = sample_params['sample_type']
        sample_rate = sample_params['sample_rate']
        if sample_type is None:
            return graph
        
        if sample_type == 'random':
            # Ensure unique time frames are sorted in ascending order.
            # unique_times = torch.unique(graph.time)
            # sorted_times, _ = torch.sort(unique_times)
            T = graph.T.sum()
            sampled_indices = []
            ...
            for t in range(T):
                ...
                n_samples = max(int(n_t * sample_rate), 1)
            ...
            sorted_idx = torch.cat(sampled_indices, dim=0)
            
            # Create a new torch_geometric Data object with the sampled nodes.
            sampled_graph = Data(
                cor=graph.cor[sorted_idx],
                time=graph.time[sorted_idx],
                feat=graph.feat[sorted_idx],
                space_emb=graph.space_emb[sorted_idx],
            ...
            # # Sample each tensor along the first dimension
            # sampled_graph = Data(
            #     'cor': graph['cor'][indices],
            #     'time': graph['time'][indices],
            #     'feat': graph['feat'][indices],
            #     'space_emb': graph['space_emb'][indices],
            #     # Retain global graph properties unchanged:
            #     'T': graph['T'],
            #     'latent_vector': graph['latent_vector']
            # )
        else:
            raise NotImplementedError(f'The sample type is {sample_type}')


class ModulatedSirenSample(ModulatedSiren):
